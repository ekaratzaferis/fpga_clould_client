...
* After integrating the design with user's testbench, the user needs to add
  these files (testbench files, toplevel wrapper file) in MIG.prj under
  synth folder.
...
* "MIG.ucf" file is the constraint file for the design. This is used by
...
* "icon4_cg.xco", "vio_async_in96_cg.xco", "vio_async_in100_cg.xco",
  ...
  MIG rtl file. In order to generate the EDIF/NGC files, you must
...
Note : When you generate the design using DEBUG_EN option, the above mentioned
       ...
       create_ise.bat files. The MIG rtl file will have the design
...
* Synth folder has the constraint file for Synplify Pro designs
  i.e. the "mem_interface_top_synp.sdc" file, "MIG.prj" file which
  has the design files to be added to the project, "script_synp.tcl" file
  that sets the synthesis options and "MIG.lso" file which is a custom
...
* For example, the design is generated with component name as MIG with
  Target FPGA LX50t-FF1136 and Compatible FPGA LX85t-FF1136.
   ...
   - Change the UCF file name in batch file to the one in compatible_ucf
     folder i.e., change the UCF name (MIG.ucf) in ise_flow.bat and
